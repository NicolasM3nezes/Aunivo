...
export async function POST(request:Request){
  ...
  if(error?.code==='23505') return NextResponse.json({message:'Sua solicitação já foi recebida.'},{status:200});
  ...
  return NextResponse.json({message:'Solicitação enviada. Entraremos em contato após a análise.'},{status:201});
}

...
export async function POST(request: Request) {
  ...
  if (claimError?.code === '23505') {
    const { data: prior } = await db.from('billing_webhook_events').select('processing_status,attempts').eq('provider_event_id', event.id).single()
  ...
  try {
    const supported = new Set(['checkout.session.completed','customer.subscription.created','customer.subscription.updated','customer.subscription.deleted','customer.subscription.trial_will_end','invoice.paid','invoice.payment_failed','invoice.payment_action_required'])
    if (supported.has(event.type)) {
      ...
      if (subscription) {
        ...
        if (event.type.startsWith('invoice.')) {
          const invoice = event.data.object as Stripe.Invoice
          await db.from('account_billing').update({ last_invoice_status: invoice.status ?? event.type }).eq('account_id', accountId)
        }
